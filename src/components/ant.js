...
export default class Comum extends React.Component{
	...
	inicia() {
        ...
        if (a !== this.state.cur) {
            // Faça algo aqui, por exemplo, chamar uma função que atualiza os componentes
			this.arr = ativas(a)
			this.grade()
			this.indices(0)
        }
	...
	grade() {
		...
		const cor = [];
		const bd = [...this.arr]
		const aux = this.state.se ? this.separa([...this.state.bd]) : [...this.state.bd]; // Evitar mutação
